  ...
  try {
    ...
    // 2. Handle STATUS UPDATE
      ...
      if (status === "Shipped" || status === "Out for Delivery" || status === "Delivered" || status === "Cancelled") {
        let msg = `Hi ${customerName}, your Lila & Co. order ${orderId} is now ${status}.`;
        
        if (status === "Shipped") msg = `Hi ${customerName}, good news! Your Lila & Co. order ${orderId} has been shipped.`;
        if (status === "Out for Delivery") msg = `Hi ${customerName}, get ready! Your Lila & Co. order ${orderId} is out for delivery.`;
        if (status === "Cancelled") msg = `Hi ${customerName}, your Lila & Co. order ${orderId} has been cancelled.`;
