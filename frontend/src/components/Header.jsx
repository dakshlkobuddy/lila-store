...
export default function Header({ store }) {
  const { route, currentUser, isAdmin, cartCount, menuOpen, setMenuOpen, go, logout, isDark, toggleDark } = store;
  ...
  return (
    <header style={{ position: "sticky", top: 0, zIndex: 30, background: isDark ? "rgba(28,23,21,.88)" : "rgba(247,241,231,.88)", backdropFilter: "blur(10px)", borderBottom: "1px solid var(--line)", transition: "background 0.25s ease" }}>
      <div style={wrap()}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 0" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button className="ec-btn ec-btn-ghost" style={{ padding: 10 }} onClick={toggleDark}>
